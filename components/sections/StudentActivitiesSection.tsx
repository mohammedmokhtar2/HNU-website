...
export function StudentActivitiesSection({
    ...
    if (error || !section) {
        return (
            <div className='py-16 text-center'>
                <p className='text-gray-500'>Failed to load student activities section</p>
            </div>
        );
    }
    ...
    if (!isStudentActivitiesContent(content)) {
        return (
            <div className='py-16 text-center'>
                <p className='text-gray-500'>Invalid student activities section content</p>
            </div>
        );
    }
                ...
                <div className='grid md:grid-cols-2 gap-8 lg:gap-12 items-stretch'>
                    ...
                    <div className='bg-white rounded-2xl shadow-xl overflow-hidden transition-transform hover:scale-105 duration-300 flex flex-col h-full'>
                        ...
                        <div className='p-6 space-y-6 flex-1 flex flex-col'>
                            ...
                            {(content.studentUnion.head?.name?.[currentLocale] || content.studentUnion.vice?.name?.[currentLocale]) && (
                            ...
                            {/* Items List */}
                            {content.studentUnion.items && content.studentUnion.items.length > 0 && (
                                <div className='space-y-3'>
                                    {content.studentUnion.items.map((item, index: number) => (
                                        ...
                                        >
                                            ...
                                            <span className='text-gray-800'>{item[currentLocale]}</span>
                                        </div>
                                    ))}
                                </div>
                            )}
                            ...
                            {content.studentUnion.link && (
                                <div className='mt-auto pt-4'>
                                    ...
                                    >
                                        <span>{currentLocale === 'ar' ? 'اعرف المزيد' : 'Learn More'}</span>
                    ...
                    <div className='bg-white rounded-2xl shadow-xl overflow-hidden transition-transform hover:scale-105 duration-300 flex flex-col h-full'>
                        ...
                        <div className='p-6 space-y-6 flex-1 flex flex-col'>
                            {/* Head and Vice Members */}
                            {(content.studentFamily.head?.name?.[currentLocale] || content.studentFamily.vice?.name?.[currentLocale]) && (
                                <div className='grid grid-cols-2 gap-4 mb-6'>
                                    {/* Head */}
                            ...
                            {/* Items List */}
                            {content.studentFamily.items && content.studentFamily.items.length > 0 && (
                                <div className='space-y-3'>
