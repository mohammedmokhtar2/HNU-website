...
export function EventConfigForm({ eventConfig, onChange, disabled = false }: EventConfigFormProps) {
...
export function EventConfigDisplay({ eventConfig, className = '' }: EventConfigDisplayProps) {
    const { formatEventDate, formatEventDuration, getEventStatusColor, getEventTypeDisplay } = useEventConfig();

    if (!eventConfig) {
        return <div className={`text-muted-foreground ${className}`}>No event configuration</div>;
    }
    ...
    return (
        <div className={`space-y-2 ${className}`}>
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="capitalize">{getEventTypeDisplay(eventConfig.eventType)}</span>
            </div>

            <div className="flex items-center gap-2">
                <span className="font-medium">Status:</span>
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getEventStatusColor(eventConfig.status)}`}>
                    {eventConfig.status.charAt(0).toUpperCase() + eventConfig.status.slice(1)}
                </span>
            </div>
            ...
            {eventConfig.eventDate && eventConfig.eventEndDate && (
                <div className="flex items-center gap-2">
                    ...
                    <span>{formatEventDuration(eventConfig.eventDate, eventConfig.eventEndDate)}</span>
                </div>
            )}
        </div>
    );
