...
export function BlogProvider({ children, initialParams = {} }: BlogProviderProps) {
    const queryClient = useQueryClient();

    // State
    const [currentBlog, setCurrentBlog] = useState<BlogWithRelations | null>(null);
    ...
    // Get blog by ID
    const getBlogById = useCallback(async (id: string): Promise<BlogWithRelations | null> => {
        try {
    ...
    // Get blog by slug
    const getBlogBySlug = useCallback(async (slug: string): Promise<BlogWithRelations | null> => {
        try {
    ...
    // Action functions
    const createBlog = useCallback(async (data: CreateBlogInput): Promise<Blog> => {
        return await createBlogMutation.mutateAsync(data);
    }, [createBlogMutation]);

    const updateBlog = useCallback(async (id: string, data: UpdateBlogInput): Promise<Blog> => {
    ...
    const toggleFeatured = useCallback(async (id: string): Promise<Blog> => {
        return await toggleFeaturedMutation.mutateAsync(id);
    }, [toggleFeaturedMutation]);

    const togglePublished = useCallback(async (id: string): Promise<Blog> => {
        return await togglePublishedMutation.mutateAsync(id);
    }, [togglePublishedMutation]);

    const reorderBlogs = useCallback(async (blogIds: string[]): Promise<void> => {
        await reorderBlogsMutation.mutateAsync(blogIds);
    }, [reorderBlogsMutation]);

    // Search and filtering functions
    const searchBlogs = useCallback(async (query: string): Promise<void> => {
        setQueryParams({ search: query, page: 1 });
    }, [setQueryParams]);

    const getBlogsByUniversity = useCallback(async (universityId: string): Promise<void> => {
        setQueryParams({ universityId, page: 1 });
    }, [setQueryParams]);

    const getBlogsByCollege = useCallback(async (collegeId: string): Promise<void> => {
        setQueryParams({ collageId: collegeId, page: 1 });
    }, [setQueryParams]);

    const getBlogsByAuthor = useCallback(async (authorId: string): Promise<void> => {
        setQueryParams({ createdById: authorId, page: 1 });
    }, [setQueryParams]);

    const getBlogsByTags = useCallback(async (tags: string[]): Promise<void> => {
...
export const useCurrentBlog = () => {
    const { currentBlog, getBlogById, getBlogBySlug, clearCurrentBlog } = useBlog();
