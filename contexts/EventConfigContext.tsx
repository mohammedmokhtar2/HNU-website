...
import { EventConfig, EventType, EventStatus, EVENT_TYPES, EVENT_STATUSES } from '@/types/event';

interface EventConfigContextType {
    // Event configuration utilities
    createEventConfig: (data: Partial<EventConfig>) => EventConfig;
    validateEventConfig: (config: EventConfig) => { isValid: boolean; errors: string[] };
...
export function EventConfigProvider({ children }: EventConfigProviderProps) {
    ...
    const validateEventConfig = (config: EventConfig): { isValid: boolean; errors: string[] } => {
        const errors: string[] = [];
    ...
    const formatEventDuration = (startDate: Date | null, endDate: Date | null): string => {
...
export const useEventConfigUtils = () => {
    const { createEventConfig, validateEventConfig, formatEventDate, formatEventDuration } = useEventConfig();
    return { createEventConfig, validateEventConfig, formatEventDate, formatEventDuration };
};

export const useEventDisplay = () => {
    const { getEventStatusColor, getEventTypeDisplay, formatEventDate, formatEventDuration } = useEventConfig();
    return { getEventStatusColor, getEventTypeDisplay, formatEventDate, formatEventDuration };
};
