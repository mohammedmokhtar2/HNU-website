...
interface MessageContextType {
    ...
    scheduleMessage: (id: string, scheduledAt: string) => Promise<MessageServiceResponse<Message>>;
    cancelScheduledMessage: (id: string) => Promise<MessageServiceResponse<Message>>;
    bulkUpdateStatus: (ids: string[], status: string) => Promise<MessageServiceResponse>;
...
export function MessageProvider({
    ...
    // Get message by ID
    const getMessageById = useCallback(async (id: string): Promise<Message | null> => {
        try {
    ...
    const createMessageMutation = useMutation({
        mutationFn: (data: CreateMessageInput) => MessageService.createMessage(data),
        onSuccess: () => {
    ...
    const createMessage = useCallback(async (data: CreateMessageInput): Promise<Message> => {
    ...
    const updateMessage = useCallback(async (id: string, data: UpdateMessageInput): Promise<Message> => {
    ...
    // Send message
    const sendMessage = useCallback(async (id: string): Promise<MessageServiceResponse<Message>> => {
        try {
    ...
    // Mark as read
    const markAsRead = useCallback(async (id: string): Promise<MessageServiceResponse<Message>> => {
        try {
    ...
    // Retry message
    const retryMessage = useCallback(async (id: string): Promise<MessageServiceResponse<Message>> => {
        try {
    ...
    // Schedule message
    const scheduleMessage = useCallback(async (id: string, scheduledAt: string): Promise<MessageServiceResponse<Message>> => {
        try {
    ...
    const cancelScheduledMessage = useCallback(async (id: string): Promise<MessageServiceResponse<Message>> => {
        try {
    ...
    // Bulk update status
    const bulkUpdateStatus = useCallback(async (ids: string[], status: string): Promise<MessageServiceResponse> => {
        try {
            ...
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Failed to bulk update messages',
            };
        }
    }, [queryClient]);

    // Bulk delete
    const bulkDelete = useCallback(async (ids: string[]): Promise<MessageServiceResponse> => {
        try {
    ...
    const filterByType = useCallback(async (type: string): Promise<void> => {
        setQueryParams({ type: type as any, page: 1 });
    }, [setQueryParams]);

    const filterByPriority = useCallback(async (priority: string): Promise<void> => {
        setQueryParams({ priority: priority as any, page: 1 });
    }, [setQueryParams]);

    const filterByDateRange = useCallback(async (dateFrom: string, dateTo: string): Promise<void> => {
