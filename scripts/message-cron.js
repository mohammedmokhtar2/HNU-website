...
async function runCronJob() {
  ...
  console.log(
    `[${startTime.toISOString()}] Starting message processing cron job...`
  );
  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      console.error(
        `[${new Date().toISOString()}] Cron job failed with status ${response.statusCode}:`
      );
    ...
    console.error(
      `[${new Date().toISOString()}] Cron job error:`,
      error.message
    );
    process.exit(1);
  }
...
async function checkStatus() {
  try {
    ...
    if (response.statusCode === 200) {
      ...
      console.error(
        `[${new Date().toISOString()}] Status check failed with status ${response.statusCode}`
      );
    }
  } catch (error) {
    console.error(
      `[${new Date().toISOString()}] Status check error:`,
      error.message
    );
  }
}
