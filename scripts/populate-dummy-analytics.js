...
function generateDummyData() {
    ...
    for (let i = 5; i >= 0; i--) {
        ...
        const monthData = Object.entries(dailyStats)
            ...
            }), { visitors: 0, pageViews: 0, sessions: 0, newVisitors: 0, returningVisitors: 0 });
    ...
    const countries = [
        'Egypt', 'United States', 'United Kingdom', 'Germany', 'France', 'Canada',
        'Australia', 'Italy', 'Spain', 'Netherlands', 'Sweden', 'Norway',
        'Japan', 'South Korea', 'China', 'India', 'Brazil', 'Mexico',
        'Saudi Arabia', 'UAE', 'Turkey', 'Russia', 'South Africa'
    ];
    ...
    // Calculate totals
    const totalVisitors = Object.values(dailyStats).reduce((sum, day) => sum + day.visitors, 0);
    const totalNewVisitors = Object.values(dailyStats).reduce((sum, day) => sum + day.newVisitors, 0);
    const totalReturningVisitors = Object.values(dailyStats).reduce((sum, day) => sum + day.returningVisitors, 0);

    // Device stats (realistic distribution)
    const deviceStats = {
        desktop: Math.floor(totalVisitors * 0.45), // 45% desktop
        mobile: Math.floor(totalVisitors * 0.50),  // 50% mobile
...
async function populateDummyData() {
    try {
        ...
        console.log(`   Returning Visitors: ${dummyData.returningVisitors.toLocaleString()}`);
        console.log(`   Device Stats: Desktop(${dummyData.deviceStats.desktop}), Mobile(${dummyData.deviceStats.mobile}), Tablet(${dummyData.deviceStats.tablet})`);
        console.log(`   Countries: ${Object.keys(dummyData.countryStats).length} countries`);
        console.log(`   Browsers: ${Object.keys(dummyData.browserStats).length} browsers`);
        console.log(`   Daily Data: ${Object.keys(dummyData.dailyStats).length} days`);
        console.log(`   Monthly Data: ${Object.keys(dummyData.monthlyStats).length} months`);
        console.log(`   Hourly Data: ${Object.keys(dummyData.hourlyStats).length} hours`);
