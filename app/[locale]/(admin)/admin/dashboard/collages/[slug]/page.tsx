...
function CollegeConfigPage() {
  ...
  const fetchPrograms = useCallback(async () => {
    ...
    try {
      ...
      const response = await ProgramService.getProgramsByCollege(
        collegeData.id,
        {
          page: 1,
          limit: 100,
    ...
    return (
      <div className='flex items-center justify-center h-64'>
        <div className='text-lg text-red-400'>
          You are not authorized to view this college
        </div>
      </div>
    );
  }
  ...
  const handleEdit = (field: string) => {
    ...
    if (field === 'name') {
      ...
      setEditingValue(
        locale === 'ar'
          ? collegeData?.description?.ar || ''
          : collegeData?.description?.en || ''
      );
    }
  };
                    ...
                    <div className='space-y-6'>
                      ...
                      <div>
                        ...
                        <div className='space-y-3'>
                          {Object.entries(collegeData?.fees || {}).map(
                            ([currency, amount]) => (
                              <div key={currency} className='flex gap-2'>
                                ...
                                <Input
                                  value={amount}
                                  onChange={e =>
                                    handleFeesUpdate(currency, e.target.value)
                                  }
                                  placeholder='Amount'
                      ...
                      <div className='flex items-center justify-between'>
                        <div>
                          ...
                          <p className='text-sm text-gray-400'>
                            Manage programs for{' '}
                            {collegeData?.name?.en || collegeData?.name?.ar}
                          </p>
                        </div>
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                              ...
                              <div className='text-center'>
                                <div className='text-2xl font-bold text-purple-400'>
                                  {
                                    programs.filter(p => p.config?.degree)
                                      .length
                                  }
                                </div>
                                <div className='text-sm text-gray-400'>
                          ...
                          {programsLoading ? (
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6'>
                              {sortedPrograms.map(program => (
                                <Card
                                  key={program.id}
                                  className='bg-gray-700 border-gray-600 hover:border-blue-500 transition-colors'
                                >
                ...
                <CardContent>
                  <div className='space-y-3'>
                    ...
                    <div>
                      ...
                      <div className='p-2 bg-gray-700 rounded text-sm text-white'>
                        {collegeData?.createdAt
                          ? new Date(collegeData.createdAt).toLocaleDateString()
                          : 'N/A'}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className='p-2 bg-gray-700 rounded text-sm text-white'>
                        {collegeData?.updatedAt
                          ? new Date(collegeData.updatedAt).toLocaleDateString()
                          : 'N/A'}
                      </div>
                    </div>
                  </div>
